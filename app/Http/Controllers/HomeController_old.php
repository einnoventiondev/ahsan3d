    ...
    public function index()
    {
        ...
        if (Auth::user()) {
            if (auth()->user()->role == 'admin') {
                ...
                return view('pages.admin.dashboard.dashboard', compact('title', 'medical', 'public','user'));
            } else {
                if(Auth::user()->approve==0){
                    ...
                    return view('pages.user.index.index', compact('counter', 'links', 'about', 'con', 'tech', 'profile', 'order', 'side', 'orders', 'logos', 'public', 'publics', 'title', 'map', 'user','products','orders_designer'));
                }
            }
        } else {
            ...
            $products = Product::orderBy('id', 'DESC')->get();
                    ...
                    $orders_designer = Order::orderBy('id','DESC')->get();
            return view('pages.user.index.index', compact('counter', 'links', 'about', 'con', 'tech', 'profile', 'order', 'side', 'orders', 'logos', 'public', 'publics', 'title', 'map', 'user','products','orders_desgner'));
        }
    }
